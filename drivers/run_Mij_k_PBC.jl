...
if length(ARGS) == 14 || length(ARGS) == 13
    ...
    if counter2 <= numk
        outputname = string(dirname, "out/Fij_PBC/", "N", N, "_d", dimension, truncstr, "_x", x, "_mg", mg, "_l0", l0, "_t", t, "_ki", k0, "_kj", ks[counter2], "_order", order, "_cutoff", cutoff, ".out")
        ...
        #println("i,j", i, " ", j)
        if i!=j
            outputname = string(dirname, "out/Fij_PBC/", "N", N, "_d", dimension, truncstr, "_x", x, "_mg", mg, "_l0", l0, "_t", t, "_ki", ks[i], "_kj", ks[j], "_order", order, "_cutoff", cutoff, ".out")
            open(outputname, "w") do file
                redirect_stdout(file) do
                    get_Fij_MPO(t, ks[i], ks[j], s, x, mg, l0, trunc, dirname, cutoff=cutoff, order=order, maxDim=maxDim, PBC=true)
                end
            end
        end
    end
