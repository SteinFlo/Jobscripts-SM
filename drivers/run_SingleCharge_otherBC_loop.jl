...
if length(ARGS) == 14
    ...
    psiname = string("singleFermion_othBC")
    ...
    outputname = string(dirname, "out/single_oBC/", "N", N, "_d", dimension, truncstr, "_X", x, "_mg", mg, "_l0", l0, "_N1-", N1, "_dt", dt, "_t", t1,"-", ts, "-", t2, "_order", order, "_cutoff", cutoff, ".out")
    ...
    open(outputname, "w") do file
        redirect_stdout(file) do
            ...
            if iseven(N1)
            psi1=applyStringMPO(N1, N,s,psi0, truncation=trunc,Q=1)
            else
            psi1=applyStringMPO(N1, N,s,psi0, truncation=trunc,Q=-1)
            end
            psiStr = normalize(apply(S ,psi1))
