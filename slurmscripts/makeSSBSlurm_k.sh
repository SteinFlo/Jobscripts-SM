...
TEscript="/data/fstein/drivers/run_SSBTE_k.jl"
GSscript="/data/fstein/drivers/run_DMRG_GS.sh"

if [ "$#" -ne 16 ]; then
    echo "Number of parameters given: "$# ", needs 16." 
    echo "Usage:" 
    echo "makeStringSS.sh N d truncation x m/g l0 N1 N2 k t1 tstep t2 order cutoff cores mem"
else
    N=$1
    d=$2
    truncation=$3
    ...
    out="/data/fstein/slurmscripts/SSBStrTE_k/S-N${N}_d${dimsymb}_x${x}_mg${mg}_l0${l0}_N1-${N1}_N2-${N2}_k${k}_t${t1}-${tstep}-${t2}_ord${order}_cut${cutoff}_${timestamp}.sh"
