...
TEscript="/data/fstein/drivers/run_SC_QuenchTE_k_PBC.jl"

if [ "$#" -ne 14 ]; then
    echo "Number of parameters given: "$# ", needs 14." 
    echo "Usage:" 
    echo "makeQuenchSS.sh N d truncation x  m/g2  l0_2  k t1 tstep t2 order cutoff cores mem"
else
    N=$1
    d=$2
    truncation=$3
    ...
    out="/data/fstein/slurmscripts/SCQuench_k_PBC/Qk_PBC-N${N}_d${dimsymb}_x${x}_mg_inf-${mg2}_l0_0-${l0_2}_k${k}_t${t1}-${tstep}-${t2}_ord${order}_cut${cutoff}_${timestamp}.sh"
    ...
    for t in $(seq $t1 $tstep $t2); do
        ...
        echo -n "singularity exec \\
                ...
                "'$container' '$driverTE' "$N $d $truncation $x   $mg2  $l0_2 $t $k $order $cutoff" '$mntpoint' >> $out
    done
